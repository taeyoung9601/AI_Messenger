...
- Oracle 선택 이유: 사원/업무/메시지 등 정형 데이터의 정합성이 중요한 관계형 구조였고, 이전 프로젝트에서 MySQL을 사용한 경험이 있어 Oracle 환경에서도 동일한 설계를 적용해보고자 선택
- Redis/RabbitMQ 미도입: 프로젝트 범위와 학습 목표를 고려해 운영/복잡도 증가를 피하고 핵심 흐름 구현에 집중

## 트러블슈팅
- 문제: 메시지 자체는 WebSocket으로 실시간 표시되지만, 채팅방 목록(최근 대화)과 초대/입장 같은 시스템 이벤트는 즉시 반영되지 않아 사용자 화면이 최신 상태와 어긋나는 문제가 있었음
- 임시 해결: 채팅방 목록은 프론트에서 주기적으로 재조회(폴링)하여 갱신
- 개선 방향: 메시지 수신/초대 이벤트 발생 시 서버에서 이벤트를 브로드캐스트하고, 프론트는 type(MESSAGE/SYSTEM/LIST_UPDATE) 기반으로 상태를 분리 업데이트하는 구조로 개선
